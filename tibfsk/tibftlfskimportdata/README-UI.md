# Kafka to FSK Migration — Web UI

An optional dashboard in the `ui/` directory for running the Kafka-to-FSK migration from a browser,
with live topology and streaming output. It is a front end that shells out to the same
`run-apachekafka-to-fsk.sh` the command line uses — the migration itself is identical either way.
...
[README.md](README.md) — start Kafka, generate the FSK config, start the FSK servers, and fill in
...
`run-apachekafka-to-fsk.sh` for you and streams the output back to the browser.
...
# 2. Generate FSK config from the demo broker properties.
...
# 3. Start FSK brokers (the realm is seeded from the generated YAML — no upload needed)
...
only if block 1 started the cluster — if you migrated from your own Kafka, stop FSK and leave the
source alone.
...
- **SVG mesh topology** — 3-node Kafka cluster (left) and 3-node FSK cluster (right); nodes turn green as soon as each process starts.
- **Animated data conduit** — canvas particle stream showing topic data flowing from Kafka to FSK in real time.
- **Real-time shell output** — WebSocket-streamed stdout/stderr from `run-apachekafka-to-fsk.sh`.
...
| `TARGET_BOOTSTRAP` | read from `kof-output/kafka-to-kof.properties` | Target FSK bootstrap addresses |
...
the environment of the `run-apachekafka-to-fsk.sh` child process. Invoking that script yourself gives you
...
The FSK cluster health check uses TCP probes on the FTL `core.servers` ports, so FSK nodes turn
...
> `5623` (`ui/server.js:72-74`, `ui/public/index.html:429-430`). `tibftlimportconfig` assigns realm
> ports *randomly* in the range 5600–5699 unless you pass `-core-servers`, so with a
> default-generated config the FSK nodes stay grey even though the cluster is healthy. Pin the
...
## Starting FSK brokers (demo helper)
