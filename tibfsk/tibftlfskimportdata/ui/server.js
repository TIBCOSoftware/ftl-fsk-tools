...
// run-apachekafka-to-fsk.sh lives one directory above ui/
const SCRIPT_PATH = path.resolve(__dirname, '../run-apachekafka-to-fsk.sh');
...
// FSK cluster: TCP health on FTL core.servers ports (5635/5620/5623) turns nodes green
// as soon as tibftlserver processes start. KafkaJS topic probe uses targetBootstrap
// and only succeeds once FSK's Kafka layer is fully initialised.
let kofCluster = {
...
function parseLine(line) {
  ...
  // Final: "Replicated 1000000 message(s) to target FSK"
  ...
  if (/Starting Kafka.*FSK/i.test(line)) {
...
server.listen(PORT, () => {
  console.log('Kafka → FSK Migration Console: http://localhost:' + PORT);
  console.log('  Source bootstrap:  ' + kafkaCluster.bootstrap);
  console.log('  Target bootstrap:  ' + kofCluster.bootstrap);
  console.log('  FSK health ports:  ' + kofCluster.nodes.map(n => n.port).join(', ') + ' (FTL core.servers)');
});
