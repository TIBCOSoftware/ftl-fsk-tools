# Kafka to FSK Migration

This tool copies all records from a source Apache Kafka cluster to a target FSK (Kafka-on-FTL)
...
> each path at **step 3**, and in step 9 stop only FSK. Every step from 3 onward is the same
...
Every command below is run **from this directory** — the one holding `run-apachekafka-to-fsk.sh`. `cd`
here first, then set:
...
`KAFKA_CLASSPATH` is read by `run-apachekafka-to-fsk.sh` and by `demo/populate-kafka.sh`; both exit 1
...
1. **FSK cannot share a port with the source Kafka.** `tibftlimportconfig` copies each source broker's
   `listeners` verbatim, so a source broker on `localhost:9092` produces a FSK broker on
   `localhost:9092` too. Both must run at the same time during a migration, so every path below
   moves FSK to `19092`/`19093`/`19094` with a `sed` command. This is only an issue when both run
   on one host; in production FSK has its own hosts and no edit is needed.
...
One Kafka broker on `localhost:9092`, one FSK server on `localhost:19092`. This is the quickest
...
### 3. Generate the FSK configuration
...
```bash
./run-apachekafka-to-fsk.sh --config ./kof-output/kafka-to-kof.properties --dry-run
```
...
```bash
./run-apachekafka-to-fsk.sh --config ./kof-output/kafka-to-kof.properties
```

Check that `planned` equals `published` in the summary.

### 8. Verify against FSK directly

```bash
bash demo/verify-kof.sh --bootstrap-server localhost:19092
```

Lists the topics on FSK, sums each one's partition end offsets to get its record count, and prints
...
> Kafka, stop FSK and leave the source cluster alone; the migration never modified it.
...
`core.servers` port pinned in step 3 — not the FSK listener port `19092`. Do not `kill` the
...
Three Kafka brokers on `localhost:9092`, `:9093`, `:9094`, and three FSK servers on `:19092`,
...
One source broker file in, one FSK server (`SRV1`, `SRV2`, `SRV3`) and one pserver out.
...
`--server` is the **FTL server** port from `core.servers` in `tibftlserver-cluster.yaml`, not the FSK
...
`5600` here is the **FTL server port** for `SRV1` from step 3, not the FSK listener port `19092`.
...
> most here: `tibftlimportconfig` translates both ZooKeeper example layouts correctly, emitting the
...
The `zookeeper.*` keys have no FSK equivalent — FSK has no ZooKeeper. They are written to
...
> from your own Kafka, stop FSK and leave the source cluster alone; the migration never modified
> it.
...
`-x` takes the **FTL server port** — `5600`, the `core.servers` port pinned in step 3 — not the FSK
...
stopping FSK — nothing about the source cluster needs to change, and it keeps serving traffic
throughout.
...
`tibftlimportconfig` emits one FSK server and one pserver per file you pass. Three files in, `SRV1`
...
it is **not** deduplicating — records already on FSK are published again. Use `--dry-run` to check
...
| `--target-bootstrap <list>` | `target.bootstrap.servers` | _(from file)_ | Target FSK bootstrap servers |
...
| `--replication-factor <n>` | `target.replication.factor` | `1` | Replication factor for topics created on FSK |
...
# Run
java -cp "build:$KAFKA_CLASSPATH" com.tibco.ftl.fsk.ApacheKafkaToFskReplicatorApp \
...
|---|---|---|
| `source.bootstrap.servers` | _(from tibftlimportconfig)_ | Source Kafka cluster addresses |
| `target.bootstrap.servers` | _(from tibftlimportconfig)_ | Target FSK cluster addresses |
| `topic.pattern` | `.*` | Regex filter for topic names |
| `include.internal` | `false` | Include Kafka internal topics (e.g. `__consumer_offsets`) |
| `target.replication.factor` | `1` | Replication factor for newly created topics on FSK |
...
| `batch.size` | `1000` | Records per partition flushed to FSK before the next batch |
...
### TLS-only example (target FSK cluster)
...
FSK pservers use PEM certificates for TLS (generated by `tibftlimportconfig` with `--tls-cert`).
If your FSK cluster is TLS-enabled, point the migration tool at the corresponding CA PEM:
