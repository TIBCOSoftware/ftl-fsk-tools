...
// UnsupportedItem is a key or key=value in the broker config that FSK does not
...
// UnsupportedScan reports the broker keys/values FSK does not support, in file
...
func UnsupportedScan(cfg *BrokerConfig) []UnsupportedItem {
	...
	for _, k := range cfg.SettingKeys {
		...
		case strings.HasPrefix(kl, "delegation.token."):
			add("delegation.token.*", "delegation tokens are not implemented in FSK")
		...
		case kl == "principal.builder.class":
			add(k, "a custom principal builder is not supported; FSK derives the principal natively")
		...
		case kl == "ssl.engine.factory.class" || strings.HasSuffix(kl, ".ssl.engine.factory.class"):
			add(k, "a custom Java SSL engine is not supported; FSK terminates TLS with OpenSSL")
		case kl == "ssl.provider" || strings.HasSuffix(kl, ".ssl.provider"):
			add(k, "a named Java security provider has no effect; FSK uses OpenSSL")
		...
		if isMechanismsKey(k) {
			if sup, unsup := mechanismsSupport(cfg.Settings[k]); len(sup) > 0 {
				for _, mu := range unsup {
					add(k+"="+mu, "FSK supports PLAIN and OAUTHBEARER only; this mechanism is ignored")
				}
			}
		}
	}
