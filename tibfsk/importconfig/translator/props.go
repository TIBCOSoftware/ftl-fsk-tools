...
const (
	...
	// FSK understands and its params are present. Not a guarantee the config works,
...
// that are not in the FSK broker properties whitelist. Only written when non-empty.
func WriteUnsupportedProperties(outputDir string, unsupportedKV []string) error {
	...
	fmt.Fprintln(f, "#")
	fmt.Fprintln(f, "# Settings in this file were unable to be automatically converted by the tool tibftlimportconfig.  No")
	fmt.Fprintln(f, "# compatible FSK settings were found to be equivalent. Not all unsupported settings will have a")
...
func writeStatusBanner(f *os.File, status ConfigStatus) {
	...
	if status == StatusInvalid {
		...
		fmt.Fprintln(f, "# Settings in this file were automatically converted by the tool tibftlimportconfig.  All")
		fmt.Fprintln(f, "# settings after conversions are compatible with FSK.  Settings that could not")
...
// nodeIDNote explains a node.id the source did not spell that way, so the
// difference between the input and the generated file is visible in the file
// itself rather than only in the tool's output.
func nodeIDNote(origin NodeIDOrigin) string {
	switch origin {
	case NodeIDFromBrokerID:
		return " (from broker.id; FSK reads the KRaft spelling node.id)"
	case NodeIDAssigned:
		return " (assigned by the tool; the source named no usable node.id)"
	}
	return ""
}
...
func writeKOFProps(f *os.File, cfg *BrokerConfig) (ConfigStatus, []string) {
	...
	fmt.Fprintf(f, "# TIBCO FTL(R) Service for Kafka Server Properties\n")
	fmt.Fprintf(f, "# Generated by conversion tool from: %s\n", cfg.SourceFile)
	fmt.Fprintf(f, "# Node ID: %d%s\n", cfg.NodeID, nodeIDNote(cfg.NodeIDOrigin))
	fmt.Fprintln(f)
...
// get the fuller note that FSK secures that traffic through the FTL servers.
func emitRejected(f *os.File, k, v string) {
	if isInterBrokerKey(k) {
		...
		fmt.Fprintf(f, "# which in FSK uses the FTL servers' own connections, not a Kafka listener.\n")
		fmt.Fprintf(f, "# Secure the FTL servers separately (realm tls.server.*/auth.providers).\n")
	} else {
		fmt.Fprintf(f, "# NOT a supported FSK property; the ftlserver rejects it. Kept for reference only.\n")
	}
	fmt.Fprintf(f, "#%s=%s\n", k, v)
}

// emitMechanisms flags a listener whose SASL mechanisms FSK cannot serve at all
...
// emitJavaKeystore flags a JKS/PKCS12 keystore type as RESOLVE-REQUIRED: FSK reads
...
// emitHandlerClass writes a SASL callback handler class key. A value FSK
