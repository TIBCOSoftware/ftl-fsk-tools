...
// index of flag groups; "tibftlimportconfig -h <group>" prints the flags in one group, and
...
func writeOverview(w io.Writer) {
	...
	fmt.Fprintln(w, "  tibftlserver-cluster.yaml         FTL pserver cluster configuration (every server, however many shards)")
	...
	fmt.Fprintln(w, `Flag groups -- run "tibftlimportconfig -h <group>" for the flags in one:`)
	width := 0
	for _, g := range flagGroups {
		if len(g.name) > width {
			width = len(g.name)
	...
	fmt.Fprintln(w, "  tibftlimportconfig -output-dir ./out server.properties")
