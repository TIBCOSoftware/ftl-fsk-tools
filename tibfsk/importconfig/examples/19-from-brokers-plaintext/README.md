...
- A running Kafka cluster — your own, or one started with the `demo/setup-kafka-kraft.sh` script
  that ships with the Kafka-to-FSK migration tool.
...
tibftlimportconfig --from-brokers localhost:9092 --output-dir output

# Three pservers, same broker config fetched once per address:
tibftlimportconfig --from-brokers localhost:9092,localhost:9093,localhost:9094 --output-dir output
```
...
1. `tibftlimportconfig` connects to each broker address in turn.
...
3. The downloaded configuration is processed through the same translation pipeline
   ...
   protocols, checking the whitelist, and generating FSK artifacts.
