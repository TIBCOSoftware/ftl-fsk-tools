...
This is still three `tibftlserver` processes on three ports. Each one hosts both a FSK pserver
and a schema pserver; the flag adds no servers and no ports.

## Verifying

Everything except the three schema daemon blocks is unchanged from example 02:

```bash
diff -r ../02-3broker-plaintext/output output
```

`ftlserver.json`, the three `kof.broker.N.properties` files and `unsupported.properties` are
identical.
