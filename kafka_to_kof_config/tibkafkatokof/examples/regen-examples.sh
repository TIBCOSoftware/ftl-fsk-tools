...
# ── 02: single-node, SASL (file-auth+tls) ─────────────────────────────────
run 02-single-node-sasl \
...
run 03-single-node-oauth \
  --core-servers "SRV1=localhost:5663" \
  "${OAUTH_COMMON[@]}" \
  server-1.properties

# ── 04: 3-broker, plaintext ───────────────────────────────────────────────
run 04-3broker-plaintext \
...
# ── 06: 3-broker, TLS only ────────────────────────────────────────────────
run 06-3broker-tls-only \
  --core-servers "SRV1=localhost:5680,SRV2=localhost:5626,SRV3=localhost:5616" \
  --tls-cert /etc/ftl/certs/server.pem \
  server-1.properties server-2.properties server-3.properties

# ── 07: 3-broker, multi-SASL (PLAIN+OAUTHBEARER) ──────────────────────────
run 07-3broker-multi-sasl \
  --core-servers "SRV1=localhost:5686,SRV2=localhost:5696,SRV3=localhost:5622" \
  "${OAUTH_COMMON[@]}" \
  server-1.properties server-2.properties server-3.properties

# ── 08: 3-broker, multi-listener ──────────────────────────────────────────
run 08-3broker-multi-listener \
...
run 09-10broker-scale \
  ...
  server-7.properties server-8.properties server-9.properties server-10.properties
...
# ── 12: 3-broker, SASL basic auth ─────────────────────────────────────────
run 12-3broker-sasl-basic \
...
# ── 15: 3-broker, SASL+mTLS ───────────────────────────────────────────────
run 15-3broker-sasl+mtls \
...
# ── 16: 3-broker, SASL+OAuth2 ─────────────────────────────────────────────
run 16-3broker-sasl+oauth2 \
...
# ── 18: 3-broker, SASL+mTLS+OAuth2 ───────────────────────────────────────
run 18-3broker-sasl+mtls+oauth2 \
