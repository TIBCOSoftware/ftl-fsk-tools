---
id: getting-started
title: Getting Started with tibkafkatokof
sidebar_label: Getting Started
---

# Getting Started with `tibkafkatokof`

`tibkafkatokof` converts Kafka broker `server.properties` files into the FTL artifacts
needed to run a KOF-enabled pserver cluster. Pass one properties file per broker; the
tool generates:
...
The `unsupported.properties` file lists any settings that have no KOF equivalent
(such as the `CONTROLLER` listener entry). Review it for reference — those settings
do not affect KOF behavior.
...
Kafka uses JKS/PKCS12 keystores; KOF reads PEM. The tool flags any keystore it finds
...
present a JWT; the tool wires the OAUTHBEARER mechanism through to KOF's OAuth2
provider.
...
`ssl.client.auth=required` maps to KOF's `tls-only` auth mode with
...
clients use OAUTHBEARER. KOF runs both auth providers concurrently.
...
different mechanism; KOF's `auth.providers` list in the secure YAML activates all of
them.
...
After editing, re-run the same `tibkafkatokof` command. The tool recomputes status on
...
Settings with no KOF equivalent (Kerberos families, delegation tokens, per-IP
