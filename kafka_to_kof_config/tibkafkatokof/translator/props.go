...
// statusMarkerPrefix is the machine-readable line the ftlserver keys on. A raw
// Kafka broker.properties has no such line, so the ftlserver treats its absence
// as "not processed by tibkafkatokof / unvalidated" and warns.
const statusMarkerPrefix = "# KOF-CONFIG-STATUS:"
...
// returns the validation status stamped into the file so the caller can set its
// exit code (StatusInvalid is a failure the operator must resolve).
func WriteKOFBrokerProperties(cfg *BrokerConfig, outputDir string, n int) (ConfigStatus, error) {
	if err := os.MkdirAll(outputDir, 0o755); err != nil {
		return StatusInvalid, fmt.Errorf("create output dir: %w", err)
	}
	path := filepath.Join(outputDir, fmt.Sprintf("kof.broker.%d.properties", n))
	f, err := os.Create(path)
	if err != nil {
		return StatusInvalid, fmt.Errorf("create %s: %w", filepath.Base(path), err)
	}
	defer f.Close()
	status := writeKOFProps(f, cfg)
	fmt.Fprintf(os.Stdout, "wrote %s [status: %s]\n", path, status)
	return status, nil
}

// configStatus reports the file's validation state: StatusInvalid if any handler
// class is unresolved or missing params, any authorizer is a custom class, or any
// keystore is JKS/PKCS12; otherwise StatusAccepted.
func configStatus(cfg *BrokerConfig) ConfigStatus {
	for _, k := range cfg.SettingKeys {
		switch {
		case isHandlerClassKey(k):
			if !resolveHandler(cfg, k).Resolved {
...
func writeKOFProps(f *os.File, cfg *BrokerConfig) ConfigStatus {
	status := configStatus(cfg)
	writeStatusBanner(f, cfg, status)
...
// writeStatusBanner emits the machine-readable status marker the ftlserver reads,
// plus a human-readable explanation. The marker line MUST be the statusMarkerPrefix
// form so both grep and the ftlserver parser can find it.
func writeStatusBanner(f *os.File, cfg *BrokerConfig, status ConfigStatus) {
	...
	case StatusAccepted:
		fmt.Fprintln(f, "# Nothing is left to resolve. Every security setting is a value KoF understands")
		fmt.Fprintln(f, "# and its params are present; settings that don't apply in KoF are kept with an")
		fmt.Fprintln(f, "# IGNORED note. This is not a guarantee the config works -- only that the tool")
		fmt.Fprintln(f, "# has nothing more to flag.")
	}
	fmt.Fprintln(f, bar)
	fmt.Fprintln(f)
}

// emitProp writes one property, dispatching the security-relevant ones to their
// handlers: SASL handler classes and authorizer.class.name are translated or
// flagged, Java keystores are flagged for conversion, and inter-broker/controller
// keys are annotated as ignored. Everything else passes through unchanged.
func emitProp(f *os.File, cfg *BrokerConfig, k string) {
	v := cfg.Settings[k]
	switch {
	case isHandlerClassKey(k):
		emitHandlerClass(f, cfg, k, v)
	case isAuthorizerKey(k):
		emitAuthorizer(f, cfg, k, v)
	case isInterBrokerKey(k):
		emitInterBroker(f, k, v)
...
// the ftlserver ignores it: that traffic is the FTL fabric, not a Kafka listener.
func emitInterBroker(f *os.File, k, v string) {
	...
	fmt.Fprintf(f, "# which in KoF is the FTL fabric, not a Kafka listener. Kept for reference only.\n")
	fmt.Fprintf(f, "# Secure the fabric separately (realm tls.server.*/auth.providers).\n")
	fmt.Fprintf(f, "%s=%s\n", k, v)
}
...
func emitHandlerClass(f *os.File, cfg *BrokerConfig, k, v string) {
	...
	if o.NeedsPick {
		...
		if o.Guess == BackendNone {
			fmt.Fprintln(f, "# Set the value to one of (ldap, oauth, file, inline) and uncomment:")
			fmt.Fprintf(f, "#%s=<ldap|oauth|file|inline>\n", k)
		} else {
			fmt.Fprintf(f, "# Name suggests %q (one of ldap, oauth, file, inline) -- verify, then uncomment:\n", o.Guess)
			fmt.Fprintf(f, "#%s=%s\n", k, o.Guess)
...
// a placeholder value and a trailing note, or a bare note (ldap/file realm config).
