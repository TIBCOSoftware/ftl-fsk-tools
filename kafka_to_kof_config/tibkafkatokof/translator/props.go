...
func WriteUnsupportedProperties(outputDir string, unsupportedKV []string) error {
	...
	defer f.Close()
	fmt.Fprintln(f, "# unsupported.properties — generated by tibkafkatokof")
	fmt.Fprintln(f, "# These properties are not in the KoF broker properties whitelist and are")
	fmt.Fprintln(f, "# not written to kof.broker.N.properties. They are kept here for reference.")
	fmt.Fprintln(f, "# Section 3 (KRaft cluster-control) keys are always rejected; security-domain")
	fmt.Fprintln(f, "# keys not in the section 1 whitelist are also excluded.")
	fmt.Fprintln(f)
...
func writeStatusBanner(f *os.File, cfg *BrokerConfig, status ConfigStatus) {
	...
	case StatusAccepted:
		...
		fmt.Fprintln(f, "# and its params are present. Properties not in the KoF whitelist were written")
		fmt.Fprintln(f, "# to unsupported.properties. This is not a guarantee the config works -- only")
		fmt.Fprintln(f, "# that the tool has nothing more to flag.")
	}
	fmt.Fprintln(f, bar)
	fmt.Fprintln(f)
}
