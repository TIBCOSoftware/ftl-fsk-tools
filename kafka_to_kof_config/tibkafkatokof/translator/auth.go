...
// class. Only classes whose behavior KoF can reproduce natively are listed;
...
//     KoF cannot run custom Java, so the operator must choose the backend;
...
// supportedMechanisms are the SASL mechanisms KoF can serve on a listener.
...
// mechanismsSupport splits a sasl.enabled.mechanisms value into the mechanisms KoF
...
// mechanismsUnservable reports a listener KoF cannot serve at all: it lists SASL
// mechanisms but none KoF supports (e.g. SCRAM-only or GSSAPI-only).
func mechanismsUnservable(value string) bool {
	sup, unsup := mechanismsSupport(value)
	return len(sup) == 0 && len(unsup) > 0
}

// recognizedAuthorizers are Kafka's built-in ACL authorizer classes. KoF
...
// resolveAuthorizer recognizes Kafka's built-in ACL authorizers (the ones KoF can
// reproduce) and returns the canonical value to write for them. Any other class is
// a custom Java authorizer KoF cannot run.
...
// (per-listener or broker-wide). KoF validates OAUTHBEARER tokens against it.
...
// oauthParams is the OAUTHBEARER validation family KoF uses, in the order to show
...
func oauthParams(handlerKey string) []paramHint {
	return []paramHint{
		{oauthKey(handlerKey, "jwks.endpoint.url"), "required -- JWKS endpoint; KoF validates token signatures against it"},
