...
Ready-to-run Kafka configurations for trying the Kafka-to-KOF migration without having to write
...
Keys with no KOF equivalent land in `unsupported.properties` — the KRaft-internal ones
...
expected: KOF uses an FTL-native quorum rather than KRaft or ZooKeeper.
