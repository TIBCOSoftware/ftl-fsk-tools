#!/usr/bin/env bash
# Verify a migration by inspecting the target KOF cluster: list its topics,
...
# KAFKA_CLASSPATH is set for run-kafka-to-kof.sh and must not leak into the
# Kafka CLI scripts.
unset CLASSPATH
...
if [[ -z "$TOPICS" ]]; then
  ...
  echo "bootstrap address — the KOF listener port, not the FTL server port."
  exit 1
fi
echo "$TOPICS" | sed 's/^/  /'
